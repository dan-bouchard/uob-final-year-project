...
if (nargin==2) % colored graph
	...
	for i=1:length(net.W.from)
        if ismember(i, net.input_spring_idx) == 0
            plot([px(from(i,1),1), px(to(i,1),1)],[py(from(i,1),1), py(to(i,1),1)],'-k','LineWidth',LINEWIDTH);hold on;
        else
            plot([px(from(i,1),1), px(to(i,1),1)],[py(from(i,1),1), py(to(i,1),1)],'-','color',[ 0.9100 0.4100 0.1700],'LineWidth',3*LINEWIDTH);hold on;
%             plot([px(from(i,1),1), px(to(i,1),1)],[py(from(i,1),1), py(to(i,1),1)],'-b','LineWidth',3*LINEWIDTH);hold on;
        end
    end
