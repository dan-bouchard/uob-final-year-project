function [net_after,sim_data] = simulate_maxwell_sys (net,input,output)
...
if (net.init_data.save_sim_data==1)
    ...
    sim_data.Fs = zeros(len,size(W.k1,1));
...
% W.x1_old = 0.5*W.dist_old;
% W.x2_old = 0.5*W.dist_old;

% W.F_old = zeros(size(W.k1));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Simulation loop
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
idx=0;  
...
%  	   d = d + net.dist_noise*rand(1,1);%-net.dist_noise*0.5;
	...
	% Calculate extensions
       % W.x1(c,1) = W.x1_old(c,1) +0.5*(d - W.dist_old(c,1));
       % W.x2(c,1) = W.x2_old(c,1) +0.5*(d - W.dist_old(c,1));
       % x1_ext = W.x1(c,1)-0.5*W.l0(c,1); % extension
       % x2_ext = W.x2(c,1)-0.5*W.l0(c,1);
       
       ext = d-W.l0(c,1);
       ext_dot = (d-W.dist_old(c,1))/time_step;
       % y = d - W.l0(c,1) - W.x1_old(c,1);
       
       fun = @(X2_dot)(W.d3(c,1)*(W.x_dot_old(c,1) - X2_dot)^3 + W.d1(c,1)*(W.x_dot_old(c,1) - X2_dot) - W.k3(c,1)*(W.x2_ext_old(c,1))^3 - W.k1(c,1)*(W.x2_ext_old(c,1)));
       % x2_ext = fzero(fun, [W.x2_ext_old(c,1)-0.05*W.l0(c,1) W.x2_ext_old(c,1)+0.05*W.l0(c,1)]);
       x2_dot = fzero(fun, W.x2_dot_old(c,1));
       x2_ext = W.x2_ext_old(c,1) + time_step*x2_dot;
       
       Fs = W.k3(c,1)*x2_ext^3+W.k1(c,1)*x2_ext;
%        x1_dot = ext_dot - x2_dot;
%        Fd = W.d3(c,1)*x1_dot^3 + W.d1(c,1)*x1_dot;
       F = Fs;
       % x1 = d - x2_ext - W.l0(c,1)/2;
       
       
       % x1_dot = (W.x1(c,1)-W.x1_old(c,1))/time_step; % d/dt extension
	   
       %x1 = d - x2_ext - W.l0(c,1)/2;
       %x1_dot = (x1 - W.x1_old(c,1))/time_step;
       
       % x2_ext = W.d1(c,1)*(d-W.l0(c,1)/2-W.x1_old(c,1))/(time_step*W.k1(c,1) + W.d1(c,1));
       % F = W.k1(c,1)*x2_ext;
       % x1 = d - x2_ext - W.l0(c,1)/2;
	   
       % F = W.k3(c,1)*x2_ext^3 + W.k1(c,1)*x2_ext;
%        Fs = W.k1(c,1)*x2_ext;
%        Fd = W.d1(c,1)*x1_ext;
%        F = 0.5*(Fs + Fd);
       
       % [F] = maxwell(W.F_old(c,1), W.k1(c,1), W.d1(c,1), ext_dot, time_step);
	   ...
	   if (net.init_data.save_sim_data==1)
        ...
        % sim_data.Fs(idx,c)  = W.k3(c,1)*x2_ext^3 + W.k1(c,1)*x2_ext;
        % sim_data.Fs(idx,c)  = W.k1(c,1)*x2_ext;
        sim_data.Fs(idx,c)  = Fs;
        % sim_data.Fd(idx,c) = W.d3(c,1)*x1_dot^3 + W.d1(c,1)*x1_dot;
        % sim_data.Fd(idx,c) = W.d1(c,1)*x1_dot;
        sim_data.F(idx,c) = F;
        sim_data.Sext(idx,c) = ext;
        sim_data.Sx2_ext(idx,c) = x2_ext;
	   end
	   % update old distance with actual distnace
	   W.dist_old(c,1) = d; % update old distance of each spring
       % W.x1_old(c,1)   = x1;
       % W.x2_old(c,1)   = W.x2(c,1);
       W.x2_ext_old(c,1) = x2_ext;
       W.x2_dot_old(c,1) = x2_dot;
       W.x_dot_old(c,1)  = ext_dot;
       % W.F_old(c,1)    = F;
    end 
