...
net.P.states(:,1) = rand_in_range(d.p_xlim,d.num); % x-positions
net.P.states(:,2) = rand_in_range(d.p_ylim,d.num); % y-positions
net.P.states(:,3) = zeros(d.num,1); % x velocity
net.P.states(:,4) = zeros(d.num,1); % y velocity
net.P.force       = zeros(d.num,2);  % force acting on the point fx,fy 
net.P.fixed       = zeros(d.num,1); % 0 for non-fixed nodes, 1 for fixed nodes
...
net.P.fixed(max_idx,1)=1; % fix the most right on % fix 2 masses
...
%  figure;imagesc(R);
w_num = sum(sum(R)); % number of connections/springs
...
net.W.kp1  = rand_in_range_exp(d.kp_lim(2,:),w_num);
net.W.kp3  = rand_in_range_exp(d.kp_lim(1,:),w_num);
...
net.W.beta = rand_in_range(d.beta_lim,w_num);       % random beta constants for Bouc-Wen hysteresis
net.W.gamma = rand_in_range(d.gamma_lim,w_num);     % random gamma constants for Bouc-Wen hysteresis
%  W.k    = 10*ones(3,1);  % for debuggin reasons
%  W.d    = 3*ones(3,1);   % for debuggin reasons
net.W.l0   = zeros(w_num,1); % resting lengths of springs
net.W.dist_old = zeros(w_num,1);
net.W.x2_ext_old = zeros(w_num,1);
net.W.z_old = zeros(w_num,1);
w_idx=0;
for i=1:d.num
   for j=1:d.num
     if(R(i,j)==1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  in_conn  = 0.2; % input connectivity percentage
net.W_in = zeros(d.num,d.nInputs);
nmax = ceil(d.num*d.in_conn);
for nI = 1:d.nInputs
   Idx = randperm(d.num);	% get random input connections
   Idx(nmax+1:end) = []; % e.g. 6 random nodes
   net.W_in(Idx,nI) = rand_in_range(d.w_in_range,nmax);	% between -iScale and +iScale [-1, 1]
end
% net.W_in is the input nodes
% net.W_in is a 30 column vector with 6 specific nodes being in range(+/-1)
...
switch d.readout_type
	...
	case 'LENGTHS'
		net.W_out = zeros(size(net.W.l0,1),d.nOutputs); % number of springs and number of outputs
		nmax = ceil(size(net.W.l0,1)*d.out_conn); % number of springs * out_conn
		for nO = 1:d.nOutputs
   			Odx = randperm(size(net.W.l0,1));	% get random output connections
   			Odx(nmax+1:end) = [];
   			net.W_out(Odx,nO) = rand_in_range(d.w_out_range,nmax);	% between -iScale and +iScale
		end
		net.readout_type = d.readout_type;
...
net.fixed_idx = find(net.P.fixed==1);  			 % indices of fixed points  (red square)
net.input_idx  = find(sum(net.W_in,2)~=0);       % indices of input neurons (green) - 
net.output_idx  = find(sum(net.W_out,2)~=0);     % indices of output neurons (green) - 
