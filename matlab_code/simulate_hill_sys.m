function [net_after,sim_data] = simulate_hill_sys(net,input,output)
...
if (net.init_data.save_sim_data==1)
    ...
    sim_data.Fsee = zeros(len,size(W.k1,1));
    sim_data.Fpee = zeros(len,size(W.k1,1));
    sim_data.F = zeros(len,size(W.k1,1));
    sim_data.Sext = zeros(len,size(W.k1,1));
    sim_data.Sx2_ext = zeros(len,size(W.k1,1));
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%  	   d = d + net.dist_noise*rand(1,1);%-net.dist_noise*0.5;
	   ...
	   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       % add input signals
       % i.e. calculate a_t Assumes the input is already scaled to [0,1]
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

       % q = (exp(W.CE.A * input(idx,:)' )-1)/(exp(W.CE.A)-1);
       % a_t = net.W_input_springs(c,1) * q;
       a_t = net.W_input_springs(c,1) * input(idx,:)';
       
	% Calculate extensions
       if ismember(c,net.input_spring_idx)
           
           x = d - W.l0(c,1); % x(i+1)
           x_dot = (d-W.dist_old(c,1))/time_step; %x_dot(i)
           x_old = W.dist_old(c,1)-W.l0(c,1); % x(i)
           x1_old = x_old - W.x2_ext_old(c,1); % x1(i)

           l_CE = W.CE.L1(c,1) + x1_old;
           opt = W.CE.opt(c,1);
           l_rel = l_CE/opt;
           C = -1/(W.CE.width.^2);
           v_max = opt/W.CE.tau_c;

           hill_func_handle = @(X2_dot) hill_force_equilib(l_CE, l_rel, C, opt, W.CE.width, v_max, W.CE.k_CE1, W.CE.k_CE2, W.CE.f_vmax, W.CE.Fmax, a_t, W.d1(c,1), W.d3(c,1), W.kp1(c,1), W.kp3(c,1), W.k1(c,1), W.k3(c,1), x_old, W.x2_ext_old(c,1), x_dot, X2_dot);
           
           x2_dot = fzero(hill_func_handle, W.x2_dot_old(c,1));
           x2 = W.x2_ext_old(c,1) + time_step*x2_dot; %x2(i+1)
           
           x1 = x-x2;
           F_see = SEE_force_hill(x2, W.k1(c,1), W.k3(c,1));
           F_pee = PEE_force_hill(x1, W.kp1(c,1), W.kp3(c,1));
           % F_see = W.k3(c,1)*x2^3 + W.k1(c,1)*x2;
           
       
       
       % W.x1(c,1) = W.x1_old(c,1) + (d - W.dist_old(c,1))/3;
       % W.x2(c,1) = W.x2_old(c,1) + 2*(d - W.dist_old(c,1))/3;
       % x1_ext = W.x1(c,1)- W.l0(c,1)/3; % extension
       % x2_ext = W.x2(c,1)- 2*W.l0(c,1)/3;
       
       % x1_dot = (W.x1(c,1)-W.x1_old(c,1))/time_step; % d/dt extension
	   
       % NEED TO INCLUDE SLACK LENGTH
           ...
           % fun = @(X2_dot)(W.d3(c,1)*(x_dot - X2_dot)^3 + W.d1(c,1)*(x_dot - X2_dot) + W.kp3(c,1)*(x_old-W.x2_ext_old(c,1))^3 + W.kp1(c,1)*(x_old-W.x2_ext_old(c,1)) - W.k3(c,1)*(W.x2_ext_old(c,1))^3 - W.k1(c,1)*(W.x2_ext_old(c,1)));
           fun = @(X2_dot)(W.d3(c,1)*(W.x_dot_old(c,1) - X2_dot)^3 + W.d1(c,1)*(W.x_dot_old(c,1) - X2_dot) + W.kp3(c,1)*(x_old-W.x2_ext_old(c,1))^3 + W.kp1(c,1)*(x_old-W.x2_ext_old(c,1)) - W.k3(c,1)*(W.x2_ext_old(c,1))^3 - W.k1(c,1)*(W.x2_ext_old(c,1)));
           kelvin_b_func_handle = @(X2_dot) kelvin_b_force_equilib(W.d1(c,1), W.d3(c,1), W.kp1(c,1), W.kp3(c,1), W.k1(c,1), W.k3(c,1), x_old, W.x2_ext_old(c,1), x_dot, X2_dot);
           
           x2_dot = fzero(fun, W.x2_dot_old(c,1)); %x2_dot(i)
           % x2_dot = fzero(kelvin_b_func_handle, W.x2_dot_old(c,1)); %x2_dot(i)
           
           x2 = W.x2_ext_old(c,1) + time_step*x2_dot; %x2(i+1)
           
           F_see = W.k3(c,1)*x2^3 + W.k1(c,1)*x2;
           F_pee = W.kp3(c,1)*(x-x2)^3 + W.kp1(c,1)*(x-x2);
           % F_see = SEE_force_hill(x2, W.k1(c,1), W.k3(c,1));
       end
	   ...
	   if (net.init_data.save_sim_data==1)
        ...
        sim_data.Fsee(idx,c)  = F_see;
        sim_data.Fpee(idx,c)  = F_pee;
        sim_data.F(idx,c)  = F;
        sim_data.Sext(idx,c) = x;
        sim_data.Sx2_ext(idx,c) = x2;
	   end
	   % update old distance with actual distnace
	   W.dist_old(c,1) = d; % update old distance of each spring
       W.x2_ext_old(c,1) = x2;
       W.x2_dot_old(c,1) = x2_dot;
       W.x_dot_old(c,1)  = x_dot;
    end 
